...
def make_all_initial_clusters(k_max=15):
    g = Yoshida.Yoshida_tree().load_igraph()
    pc = peak_clusters.peak_clusters(FDR=1E-3)
    for k in range(11, k_max+1):
      print(["generating initial clusters for k ", k])
      ic = initial_clusters(k, g, pc)
      ic.generate_initial_clusters(nrand=100, save_results=True)
      

class initial_clusters:
    
    col_cluster_directory = conf.DATA_DIR + 'cell_type_clusters/'
    ...
    def __init__(self, k, g, peak_cluster, 
        ...
        self.col_cluster_file = self.col_cluster_directory + str(k) \
                                    + '_initial'  + str(k) + '.csv'
        ...
        self.rootnode = Yoshida.Yoshida_tree().get_root_vertex(return_name=False)
    ...
    # as well as random clusterings, the different combination of clusterings
    #  generated by the qualified vertices are returned
    def generate_initial_clusters(self, nrand, save_results=True):
        self.results = []
    ...
    def load_results(self):
        return pd.read_csv(self.col_cluster_file, sep=",")
    ...
    def save_results(self):
        ...
        tb.insert(tb.shape[1], "K", self.num_clusters)
